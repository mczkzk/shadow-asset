...
| 項目 | 算出方法 | 意味 |
|---|---|---|
| 生活防衛資金 | 生活費 × Nヶ月 (選択式: 0/3/6/9/12、デフォルト6) | 収入が途絶えた時のバッファ |
| 現金ポジション | 生活費 × Nヶ月 (選択式: 0/6/12/24/36) | 暴落時に株を取り崩さず生活するためのバッファ |
| 個人向け国債 | 生活費 × Nヶ月 (選択式: 0/6/12/24/36) | 安全資産による追加バッファ |
| FIRE目標額 | 生活費 × 12ヶ月 × 25年 (4%ルール) | 資産収入だけで生活できる額 |

#### 表示 (アロケーションページ)
- 各項目について: 目標額、現在額、過不足(金額)を表示
- 生活防衛資金: `manual_assets`の生活防衛資金合計と比較
- 現金ポジション: アロケーションの「現金」クラス合計と比較
- 個人向け国債: アロケーションの「個人向け国債」クラス合計と比較
- FIRE目標額: 総資産(生活防衛資金除く)と比較
- 生活費未設定時は目標判定セクション非表示

#### データ保存
- `settings`テーブル(key-value): `monthly_expense` キーに万円単位で保存、`emergency_fund_months`/`cash_position_months`/`gov_bond_months` キーに月数を保存
- フロントエンドで直接編集可能(生活費: インライン入力、月数: セレクトボックス)
