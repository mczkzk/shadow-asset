...
- **取り込む列**: 日付, 合計, 預金・現金・暗号資産, 株式(現物), 投資信託, 債券, 保険, 年金 → これらを資産クラスにマッピング
- **マッピング**: 株式(現物) → 株式、投資信託 + 年金 → 投資信託、債券 → 債券、預金・現金・暗号資産 → 現金、保険 → 保険
- **取り込まない列**: ポイント, その他の資産 (このアプリの管理対象外)
- **既存データ優先**: 同一日付のsnapshotsが存在する場合、既存データを優先
- **UIの位置**: ダッシュボードの資産推移チャート横にCSV取込ボタン
- **チャート表示**: 積み上げエリアチャート (商品種別の内訳)。MFデータのみの日は株式/債券/現金/保険、snapshotがある日はフル内訳
