...
6. **アセットアロケーション**: 資産クラス別ドーナツチャート+配分詳細テーブル。手入力資産(現金/外貨預金/不動産/保険/生活防衛資金)のCRUD。外貨預金は為替自動換算。`holding_snapshots`の最新データを使用し外部API呼び出しなし(外貨為替のみ)。生活防衛資金はチャート・テーブルから除外し注記表示(配分対象外)。生活費ベースの目標判定あり(後述)
...
#### 手入力資産クラス
`manual_assets`の対応クラス: 現金, 個人向け国債, 外貨預金, 不動産, 保険, 生活防衛資金
