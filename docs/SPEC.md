...
| 商品種別 | 対象 |
|---|---|
| 投資信託 | オルカン, S&P500等のインデックスファンド, DC投信 |
| 株式 | 個別米国株, ETF全般 (GLDM等の金ETFも含む) |
| ゴールド | 金貨, 金地金 |
| 暗号資産 | BTC, ETH, BCH |
| 債券 | 債券ファンド/ETF (将来用) |

### 資産クラス (asset_class) = アセットアロケーションページ (将来)
「リスク配分がどうなっているか」の戦略判断用。

| 資産クラス | 算出方法 |
|---|---|
| 株式 | 投資信託(株式ファンド) + 個別株 + 株式ETF |
| 債券 | 債券ファンド/ETF |
...
## MVP Scope (全て実装済み)
1. **ダッシュボード**: 総資産額、商品種別内訳(ドーナツチャート)、口座別リスト
...
- **取り込む列**: 日付, 合計, 株式(現物), 投資信託, 債券, 年金 → これらを資産クラスにマッピング
- **マッピング**: 株式(現物) + 投資信託 + 年金 → 株式、債券 → 債券
- **取り込まない列**: 預金・現金・暗号資産, 保険, ポイント, その他の資産 (このアプリの管理対象外)
- **既存データ優先**: 同一日付のsnapshotsが存在する場合、既存データを優先
- **UIの位置**: ダッシュボードの資産推移チャート横にCSV取込ボタン
- **チャート表示**: 積み上げエリアチャート (商品種別の内訳)。MFデータのみの日は株式/債券の2色、snapshotがある日はフル内訳

### 未実装

#### 機能
- **MoneyForward CSV取込**: 資産推移の過去データ補完 + 積み上げエリアチャート (作業中)
- **アセットアロケーションページ**: 資産クラス別ビュー、現金/生活防衛資金の手入力、4%ルール判定
- **暴落時メンタルガードモード**: 前日比表示 + 暴落時に非表示切替 (パニック売り防止)
- **積立シミュレーション**: 将来の資産予測
- **損益計算**: 取得価額との比較
