...
暴落時にパニック売りを防ぐため、前日比の表示/非表示を切り替える機能。

- **トグルスイッチ**: ダッシュボード右上「更新」ボタン下に配置。「前日比」ラベル付き
- **対象**: TotalAssetsの総額前日比 + AccountListの個別銘柄前日比
- **自動判定**: 前日比が-5%以下の場合、デフォルトで非表示 (トグルOFF)
- **状態管理**: React state のみ (アプリ再起動でリセット)
