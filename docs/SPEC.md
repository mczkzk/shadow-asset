...
6. **アセットアロケーション**: 資産クラス別ドーナツチャート+配分詳細テーブル。手入力資産(現金/外貨預金/不動産/保険/公的年金/生活防衛資金)のCRUD。外貨預金は為替自動換算。`holding_snapshots`の最新データを使用し外部API呼び出しなし(外貨為替のみ)。生活防衛資金と公的年金はチャート・テーブルから除外し注記表示(配分対象外)。生活費ベースの目標判定あり(後述)
...
月の生活費(1つの入力値)から、4つの目標額を自動算出して現状との過不足を表示する。

#### 入力
- **月の生活費**: ユーザーが手入力(万円単位)。`settings`テーブルに保存
...
| 項目 | 選択肢 | デフォルト | 意味 |
|---|---|---|---|
| 生活防衛資金 | 0/3/6/9/12 | 6 | 収入が途絶えた時のバッファ |
| 現金ポジション | 0/6/12/24/36 | 12 | 暴落時に株を取り崩さず生活するためのバッファ |
| 個人向け国債 | 0/6/12/24/36 | 0 | 安全資産による追加バッファ |

**FIRE目標額**: 生活費 × 12ヶ月 × 25年 (4%ルール)。超えれば超えるほどOK。

**割合ベース目標** (総資産の N%)

| 項目 | 入力 | 意味 |
|---|---|---|
| 債券 | 0-100% (テキスト入力) | 債券ETF (AGG等) の配分目標 |
| ゴールド | 0-100% | ゴールド (現物+ETF) の配分目標 |
| 暗号資産 | 0-100% | 暗号資産の配分目標 |
| 外貨預金 | 0-100% | 外貨預金の配分目標 |
| 不動産 | 0-100% | 不動産の配分目標 |
| 保険 | 0-100% | 保険の配分目標 |

- 空欄 = 判定しない、0 = 持たない目標 (保有があれば警告)
- 割合の母数は総資産から生活防衛資金を除いた額

#### 判定表示 (トラフィックライト)

- **FIRE目標額のみ**: 超えればOK (緑/赤の2色)
- **その他すべて**: 目標値からの乖離率で3色判定
  - ±10%以内 → 緑 (OK)
  - ±25%以内 → 黄 (要調整)
  - ±25%超 → 赤 (大幅乖離)
- **ノイズフィルタ**: 乖離の絶対額が総資産の0.5%未満なら緑扱い (小額ポジションの変動を無視)
- 生活費未設定時は目標判定セクション非表示

#### データ保存
- `settings`テーブル(key-value):
  - `monthly_expense`: 月の生活費 (万円単位)
  - `emergency_fund_months`/`cash_position_months`/`gov_bond_months`: 月数
  - `bond_target_pct`/`gold_target_pct`/`crypto_target_pct`/`forex_deposit_target_pct`/`insurance_target_pct`/`real_estate_target_pct`: 目標配分割合 (0-100、空=判定しない)
- フロントエンドで直接編集可能(生活費: インライン入力、月数: セレクトボックス、割合: テキスト入力)

#### 手入力資産クラス
`manual_assets`の対応クラス: 現金, 個人向け国債, 外貨預金, 不動産, 保険, 公的年金, 生活防衛資金

- **公的年金**: 年金ネットで確認できる「これまでの保険料納付総額(総合計)」を手入力で記録するための専用クラス。総資産には含めるが、流動性ゼロのためアセットアロケーションチャート/FIRE目標母数/割合ベース判定母数の全てから除外（生活防衛資金と対称）。名前は固定「公的年金」(自動入力)。
- **配分対象外クラス**: 生活防衛資金, 公的年金
