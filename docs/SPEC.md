...
月の生活費(1つの入力値)から、3つの目標額を自動算出して現状との過不足を表示する。
...
#### データ保存
- `settings`テーブル(key-value): `monthly_expense` キーに万円単位で保存、`cash_position_months` キーに月数を保存
- フロントエンドで直接編集可能(生活費: インライン入力、現金月数: セレクトボックス)
